...
void UserPswd_Set_Encry_Status(uint8_t flag){
	...
	isEncryed = flag;	
	b_Eeprom_Check_Conn();	
	b_Eeprom_Write_Byte(EEP_ADD_IS_ENCRYED,isEncryed); 
...
uint8_t UserPswd_Save_EncryToFlash(){
	b_Eeprom_Check_Conn();
	b_Eeprom_Write_Buf(EEP_ADD_SEND_PASSWORD,localEncryPasswd,MAX_PASSWD_LEN); 
	return 1;	
}
uint8_t UserPswd_Read_EncryFromFlash(){
	b_Eeprom_Check_Conn();	
	b_Eeprom_Read_Buf(EEP_ADD_SEND_PASSWORD,localEncryPasswd,MAX_PASSWD_LEN); 
	localEncryCount = 0;
	int i = 0;
