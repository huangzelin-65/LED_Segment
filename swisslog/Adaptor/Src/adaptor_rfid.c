...
void vCarRfid_Start_DMA_Receive(uint8_t* ucCarRfid_Rx_Buffer) {
    v_Swisslog_Start_DMA_Receive(&huart5, ucCarRfid_Rx_Buffer, CAR_RFID_RX_BUF_SIZE);
}


// 获取当前DMA接收的数据长度
uint32_t ulCarRfid_Get_DMA_Receive_Len(uint32_t* pu32_startIdx)
{
    static uint32_t u32_currentLen= 0;
    static uint32_t u32_lastLen = 0;
    uint32_t u32_len = 0 ;
    if (pu32_startIdx != NULL) {
        *pu32_startIdx = u32_lastLen % CAR_RFID_RX_BUF_SIZE;; // 起始位置 = 上一次的累计长度
    }

    // 计算当前DMA累计长度
    u32_currentLen = CAR_RFID_RX_BUF_SIZE - __HAL_DMA_GET_COUNTER(huart5.hdmarx);

    // 计算单包长度（缓冲区绕回/没有绕回）
    if(u32_currentLen > u32_lastLen) {
        u32_len = u32_currentLen - u32_lastLen ;
    } else {
        u32_len = (CAR_RFID_RX_BUF_SIZE - u32_lastLen) + u32_currentLen;
    }
