        ...
        switch(MotionRecv_msg)
        {
          ...
          case CarRunning:
            ...
            if(CarCheckFlagobj.ToggleSwtichPosition == ToggleFront)
            {
              ...
              {
                DEBUGINFO_ALL("remote Auto mode\r\n");

                // 电机未使能运行（通过wifi指令下发）
                if(CarRunStatus_obj.MotorEnable == MotorDisable)
                {
                  DEBUGINFO_ALL("ReadyToRun\r\n");
              ...
              {
                DEBUGINFO_ALL("remote Manual mode\r\n");
                // 电机按普通速度运行（手动档下），方向不变
                CarRunStatus_obj.IsCarRunning = CarRunning;
            ...
            {
              DEBUGINFO_ALL("local Manual mode\r\n");
              // 预设值向前
              if(CarRunStatus_obj.SetDirection == Forward)
              {
